...
import spout.KafkaConsumerUtil.Host;
...
public class KafkaSpoutConfig implements Serializable {
    ...
    private String getConfig(String key, String defaultValue) {
        ...
        /*if (properties != null && properties.containsKey(key)) {
            ...
            return String.valueOf(stormConf.get(key)).replaceAll("\"", "");
        } else {
            return defaultValue;
        }*/
        if(stormConf.containsKey(key)){
            System.out.println("Contain the key : " + key);
            System.out.println("Result is : "+String.valueOf(stormConf.get(key)));
    ...
    public List<Host> convertHosts(String hosts, int defaultPort) {
        ...
        for (String s : hostArr) {
            ...
            if (spec.length == 1) {
                ...
                host = new Host(spec[0], Integer.parseInt(spec[1].replaceAll("^\"", "")));
            } else {
