...
+ function ($) {
  ...
  var Button = function (element, options) {
    ...
    this.options = $.extend({}, Button.DEFAULTS,
      options)
  }
  ...
  Button.prototype.setState = function (state) {
    ...
    if (!data.resetText) $el.data('resetText',
      $el[val]())
    ...
    if ($parent.length) {
      ...
      if ($input.prop('type') === 'radio') {
        ...
        if ($input.prop('checked') && this.$element
          ...
          $parent.find('.active').removeClass(
            'active')
      }
      if (changed) $input.prop('checked', !this
        .$element.hasClass('active')).trigger(
        'change')
    }

    if (changed) this.$element.toggleClass(
      'active')
  }
  ...
  $(document).on('click.bs.button.data-api',
    ...
    function (e) {
      var $btn = $(e.target)
      if (!$btn.hasClass('btn')) $btn = $btn.closest(
        '.btn')
      $btn.button('toggle')
      e.preventDefault()
    })
...
+ function ($) {
  ...
  var Carousel = function (element, options) {
    ...
    this.options.pause == 'hover' && this.$element
      .on('mouseenter', $.proxy(this.pause,
        this))
      .on('mouseleave', $.proxy(this.cycle,
        this))
  }
  ...
  Carousel.prototype.cycle = function (e) {
    ...
    this.options.interval && !this.paused && (
      this.interval = setInterval($.proxy(
  ...
  Carousel.prototype.to = function (pos) {
    ...
    return this.slide(pos > activeIndex ?
      'next' : 'prev', $(this.$items[pos]))
  }
  ...
  Carousel.prototype.pause = function (e) {
    ...
    if (this.$element.find('.next, .prev').length &&
      ...
      this.$element.trigger($.support.transition
        .end)
      this.cycle(true)
    }
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    if (this.$indicators.length) {
      this.$indicators.find('.active').removeClass(
        'active')
      this.$element.one('slid.bs.carousel',
        function () {
          var $nextIndicator = $(that.$indicators
            .children()[that.getActiveIndex()]
          )
    ...
    if ($.support.transition && this.$element.hasClass(
      ...
      $active
        .one($.support.transition.end, function () {
          $next.removeClass([type, direction]
  ...
  $.fn.carousel = function (option) {
    return this.each(function () {
      ...
      var options = $.extend({}, Carousel.DEFAULTS,
        $this.data(), typeof option ==
        'object' && option)
      var action = typeof option ==
        'string' ? option : options.slide

      if (!data) $this.data('bs.carousel', (
        data = new Carousel(this,
          options)))
      if (typeof option == 'number') data.to(
        option)
  ...
  $(document).on('click.bs.carousel.data-api',
    ...
    function (e) {
      ...
      var options = $.extend({}, $target.data(),
        $this.data())
      var slideIndex = $this.attr(
        'data-slide-to')
      if (slideIndex) options.interval = false

      $target.carousel(options)

      if (slideIndex = $this.attr(
          'data-slide-to')) {
        $target.data('bs.carousel').to(
          slideIndex)
      }
...
+ function ($) {
  ...
  var Collapse = function (element, options) {
    this.$element = $(element)
    this.options = $.extend({}, Collapse.DEFAULTS,
      options)
    this.transitioning = null

    if (this.options.parent) this.$parent = $(
      this.options.parent)
  ...
  Collapse.prototype.dimension = function () {
    var hasWidth = this.$element.hasClass(
      'width')
    return hasWidth ? 'width' : 'height'
  }

  Collapse.prototype.show = function () {
    if (this.transitioning || this.$element.hasClass(
        'in')) return
    ...
    if (!$.support.transition) return complete.call(
      this)
  ...
  Collapse.prototype.hide = function () {
    ...
    this.$element[dimension](this.$element[
      dimension]())[0].offsetHeight
  ...
  $(document).on('click.bs.collapse.data-api',
    ...
    function (e) {
      var $this = $(this),
        href
      var target = $this.attr('data-target') ||
        e.preventDefault() || (href = $this.attr(
          'href')) && href.replace(
          /.*(?=#[^\s]+$)/, '') //strip for ie7
      var $target = $(target)
...
+ function ($) {
  ...
  Dropdown.prototype.keydown = function (e) {
    ...
    if (!isActive || (isActive && e.keyCode ==
        27)) {
    ...
    var index = $items.index($items.filter(
      ':focus'))

    if (e.keyCode == 38 && index > 0) index-- // up
      if (e.keyCode == 40 && index < $items.length -
        1) index++ // down
        if (!~index) index = 0
...
+ function ($) {
  ...
  var Modal = function (element, options) {
    ...
    if (this.options.remote) this.$element.load(
      this.options.remote)
  }
  ...
  Modal.prototype.show = function (_relatedTarget) {
    ...
    this.$element.on('click.dismiss.modal',
      '[data-dismiss="modal"]', $.proxy(this.hide,
        this))
  ...
  Modal.prototype.enforceFocus = function () {
    $(document)
      ...
      .on('focusin.bs.modal', $.proxy(function (
        e) {
  ...
  $.fn.modal = function (option, _relatedTarget) {
    return this.each(function () {
      ...
      if (!data) $this.data('bs.modal', (
        data = new Modal(this, options)
      ))
      if (typeof option == 'string') data[
        option](_relatedTarget)
...
+ function ($) {
  ...
  Tooltip.prototype.init = function (type,
    element, options) {
    this.enabled = true
    this.type = type
    this.$element = $(element)
    ...
    for (var i = triggers.length; i--;) {
      ...
      if (trigger == 'click') {
        this.$element.on('click.' + this.type,
          this.options.selector, $.proxy(this
            .toggle, this))
  ...
  Tooltip.prototype.enter = function (obj) {
    ...
    if (!self.options.delay || !self.options.delay
  ...
  Tooltip.prototype.show = function () {
    ...
    if (this.hasContent() && this.enabled) {
      ...
      if (this.options.animation) $tip.addClass(
        'fade')

      var placement = typeof this.options.placement ==
        'function' ?
        this.options.placement.call(this, $tip[
          0], this.$element[0]) :
        this.options.placement

      var autoToken = /\s?auto?\s?/i
      var autoPlace = autoToken.test(placement)
      if (autoPlace) placement = placement.replace(
        autoToken, '') || 'top'
      ...
      this.applyPlacement(calculatedOffset,
        placement)
  ...
  Tooltip.prototype.applyPlacement = function (
    ...
    var marginTop = parseInt($tip.css(
      'margin-top'), 10)
    var marginLeft = parseInt($tip.css(
      'margin-left'), 10)
    ...
    if (/bottom|top/.test(placement)) {
      ...
      this.replaceArrow(delta - width +
        actualWidth, actualWidth, 'left')
    } else {
      this.replaceArrow(actualHeight - height,
        actualHeight, 'top')
    }
  ...
  Tooltip.prototype.replaceArrow = function (
    delta, dimension, position) {
    this.arrow().css(position, delta ? (50 * (1 -
      delta / dimension) + "%") : '')
  }
  ...
  Tooltip.prototype.fixTitle = function () {
    var $e = this.$element
    if ($e.attr('title') || typeof ($e.attr(
        'data-original-title')) != 'string') {
      $e.attr('data-original-title', $e.attr(
        'title') || '').attr('title', '')
    }
  }
  ...
  Tooltip.prototype.getCalculatedOffset =
    function (placement, pos, actualWidth,
      actualHeight) {
  ...
  Tooltip.prototype.getTitle = function () {
    ...
    title = $e.attr('data-original-title') || (
      typeof o.title == 'function' ? o.title.call(
        $e[0]) : o.title)
...
+ function ($) {
  ...
  if (!$.fn.tooltip) throw new Error(
    'Popover requires tooltip.js')

  Popover.DEFAULTS = $.extend({}, $.fn.tooltip.Constructor
    .DEFAULTS, {
      placement: 'right',
      trigger: 'click',
      content: '',
  ...
  Popover.prototype.getContent = function () {
    ...
    return $e.attr('data-content') || (typeof o
      .content == 'function' ?
      o.content.call($e[0]) :
      o.content)
  }
...
+ function ($) {
  ...
  ScrollSpy.prototype.refresh = function () {
    var offsetMethod = this.$element[0] ==
    ...
    var $targets = this.$body
      ...
      .map(function () {
        var $el = $(this)
        var href = $el.data('target') || $el.attr(
          'href')
        var $href = /^#\w/.test(href) && $(
          href)
  ...
  ScrollSpy.prototype.process = function () {
    ...
    var maxScroll = scrollHeight - this.$scrollElement
      .height()
    ...
    for (i = offsets.length; i--;) {
      activeTarget != targets[i] && scrollTop >=
        offsets[i] && (!offsets[i + 1] ||
          scrollTop <= offsets[i + 1]) && this.activate(
          targets[i])
    }
  }
  ...
  Tab.prototype.activate = function (element,
    ...
    function next() {
      ...
      if (element.parent('.dropdown-menu')) {
        element.closest('li.dropdown').addClass(
          'active')
      }
...
+ function ($) {
  ...
  var Affix = function (element, options) {
    this.options = $.extend({}, Affix.DEFAULTS,
      options)
    this.$window = $(window)
      .on('scroll.bs.affix.data-api', $.proxy(
        this.checkPosition, this))
      .on('click.bs.affix.data-api', $.proxy(
  ...
  Affix.prototype.checkPositionWithEventLoop =
    function () {
      setTimeout($.proxy(this.checkPosition, this),
        1)
    }
  ...
  Affix.prototype.checkPosition = function () {
    ...
    this.affixed = affix
    this.unpin = affix == 'bottom' ? position.top -
      scrollTop : null
  ...
  $(window).on('load', function () {
    $('[data-spy="affix"]').each(function () {
      ...
      if (data.offsetBottom) data.offset.bottom =
        data.offsetBottom
      if (data.offsetTop) data.offset.top =
        data.offsetTop
